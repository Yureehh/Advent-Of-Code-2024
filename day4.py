...
def count_xmas_occurrences(grid, word="XMAS"):
    directions = [
        ...
        (-1, -1), (-1, 1), (1, -1), (1, 1)  # diagonal: all directions
    ]
            ...
            if grid[row][col] == word[0]:  # Found the starting letter
                for dx, dy in directions:
                    ...
                    for i in range(word_length):
                        ...
                        if not (is_valid_position(nx, ny, rows, cols) and grid[nx][ny] == word[i]):
                            match = False
                            break
                    if match:
                        count += 1
    ...
    for row in range(1, rows - 1):  # Center row of the X-MAS
        for col in range(1, cols - 1):  # Center column of the X-MAS
            if grid[row][col] == "A":  # Center of the X-MAS
                ...
                if (
                    is_valid_position(row - 1, col - 1, rows, cols) and grid[row - 1][col - 1] in ("M", "S") and
                    is_valid_position(row + 1, col + 1, rows, cols) and grid[row + 1][col + 1] in ("M", "S") and
